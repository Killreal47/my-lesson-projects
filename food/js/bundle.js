...
function calc() {
	...
	function calcTotal() { // Функция расчитывает количество калорий
	...
	function getStaticInformation(selector, activeClass) { // принимаем значения пола и активности, добавляем класс активности
		const elements = document.querySelectorAll(selector);
	...
	function getDynamicInformation(selector) { // принимаем значения веса, роста и возраста
...
function cards() {
	...
	class MenuCart {
		...
		changeToUAH() { // стоимость поступает в $, преобразуем в гривны
			this.price = this.price * this.transfer;
		}

		render() { // создаем нашу верстку карточек на странице "img/tabs/vegy.jpg"
...
function forms(formSelector, modalTimerId) {
	...
	function bindPostData(form) { //отвечает за привязку постинга
		form.addEventListener('submit', (e) => {
			...
			// request.setRequestHeader('Content-type', 'application/json; charset=utf-8'); // говорим серверу какие данные он получает 
...
function slider({ container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field }) {
	//***************************************************************************** */
	//                    Слайдер простой вариант, просто появляются
	//***************************************************************************** */
	...
	// function showSlides(n) { // функция показывающая слайды
	...
	// function plusSlides(n) { // Увеличиваем или уменьшаем индекс; n = -1/1
	...
	//***************************************************************************** */
	//                 Слайдер сложный вариант, в виде карусели
	//***************************************************************************** */

	const slides = document.querySelectorAll(slide), // каждый отдельный слайд
		slider = document.querySelector(container), // весь блок слайдера
		...
		slidesWrapper = document.querySelector(wrapper), // внешняя обертка, через которую показывается слайды(окошко)
		slidesField = document.querySelector(field), // поле наших слайдов, которое двигается
		width = window.getComputedStyle(slidesWrapper).width;	// понимаем какую ширину занимает окошко (slidesWrapper), через которое показывается слайд

	let slideIndex = 1; // индекс текущего слайда 
	let offset = 0; // отступ вправо или влево
	...
	slides.forEach(slide => { // делаем, чтобы была одинаковая ширина у всех картинок 650px
